      ...
      <div className="p-4 sm:p-6 flex-grow flex flex-col">
        ...
        {status && (
          <div className="mt-3">
            <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-mono ${
              status === 'live' ? 'bg-green-500/20 text-green-400 border border-green-500/40' :
              status === 'development' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/40' :
              status === 'done' ? 'bg-gray-500/20 text-gray-400 border border-gray-500/40' :
              'bg-yellow-500/20 text-yellow-400 border border-yellow-500/40'
            }`}>
              {status === 'live' ? 'En Vivo' :
               status === 'development' ? 'En Desarrollo' :
               status === 'done' ? 'Completado' : 'Prototipo'}
            </span>
          </div>
        )}
          ...
          <div className="flex gap-3 mt-2">
            {url && (
              <a href={url} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-xs text-purple-light hover:text-purple-primary transition-colors"
              >
                🌐 Demo
              </a>
            )}
        ...
        <div className="flex flex-col gap-2 mt-auto pt-3 sm:pt-4">
          ...
          >
            ...
            <span className="truncate">Ver Imagenes ({screenshots?.length || 0})</span>
          </button>
          <button 
            onClick={(e) => {
              e.stopPropagation()
